  ...
  client.on("messageCreate", async (message) => {
    ...
    if (autoResponseSettings && autoResponseSettings.name) {
      ...
      if (autoResponseCooldown.has(message.author.id))
        return message.channel.sendCustom(
          `${message.client.emoji.fail} Desacelera - ${message.author}`
        );
    ...
    if (
      ...
      return message.channel.sendCustom(command).catch((e) => {
        message.channel.sendCustom(
          `Hubo un problema al enviar su embed, que probablemente sea un error JSON.\nLea más aquí --> https://roxxy.es/embeds\n\n__Error:__\n\`${e}\``
        );
      });
    }
  });
