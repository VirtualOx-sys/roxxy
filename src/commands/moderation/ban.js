...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      name: "ban",
      aliases: ["b"],
      description: "Prohibe al usuario especificado de su servidor Discord.",
      category: "Moderacion",
      usage: "<usuario> [razón]",
      examples: ["ban @VirtualOx ¡Rompiendo las reglas!"],
      guildOnly: true,
  ...
  async run(message, args) {
      ...
      await client.users
        ...
        .then(async (u) => {
          ...
          await message.guild.members.ban(u.id, {
            reason: `${reason} / Usuario responsable: ${message.author.tag}`,
          });
                  ...
                  if (
                    ...
                    if (logging.moderation.ban == "true") {
                      ...
                      const logEmbed = new MessageEmbed()
                        .setAuthor(
                          `Acción: \`Ban\` | ${u.tag} | Caso #${logcase}`,
                          u.displayAvatarURL({ format: "png" })
                        )
                        .addField("Usuario", u, true)
    ...
    await member
      .ban({ reason: `${reason} / Usuario responsable: ${message.author.tag}` })
      .catch((err) =>
    ...
    if (
      ...
      if (logging.moderation.ban_action === "2") {
        dmEmbed = `${message.client.emoji.fail} Has sido baneado en **${message.guild.name}**`;
          ...
          if (message.channel.id !== logging.moderation.ignore_channel) {
            if (
              ...
              if (logging.moderation.ban == "true") {
                ...
                const logEmbed = new MessageEmbed()
                  .setAuthor(
                    `Acción: \`Ban\` | ${member.user.tag} | Caso #${logcase}`,
