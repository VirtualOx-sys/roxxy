...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!args[0])
      return message.channel.sendCustom(`${language.kill1}`).catch(() => {
        message.channel.sendCustom(`${language.kill1}`);
      });

    let userr = message.guild.members.cache.get(message.mentions.users.first());
    if (!userr) return message.channel.sendCustom(`${language.kill1}`);
    ...
    const answers = [
      ...
      `${message.author.username} aplastó a ${user} ${language.kill28}`,
    ...
    if (userr.id === message.author.id)
      return message.channel.sendCustom(`${language.kill2}`).catch(() => {
        message.channel.sendCustom(`${language.kill1}`);
      });

    message.channel
      .sendCustom(`${answers[Math.floor(Math.random() * answers.length)]}`)
      .catch(() => {
        message.channel.sendCustom(`${language.kill1}`);
      });
  }
