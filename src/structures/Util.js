...
module.exports = class Util {
  ...
  async loadCommands() {
    for await (const commandFile of this.loadFiles(
      ...
      if (!this.isClass(File))
        throw new TypeError(`El comando ${name} no exporta una clase.`);
      const command = new File(this.client, name.toLowerCase());
      if (!(command instanceof Command))
        throw new TypeError(`Comando ${name} no pertenece a Comandos.`);
  ...
  async loadEvents() {
    for await (const eventFile of this.loadFiles(
      ...
      if (!this.isClass(File))
        throw new TypeError(`El evento ${name} no exporta una clase!`);
      const event = new File(this.client, name);
      if (!(event instanceof Event))
        throw new TypeError(`El evento ${name} no pertenece a los eventos`);
