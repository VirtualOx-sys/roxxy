          ...
          if (logging.server_events.channel_created == "true") {
            if (message.type === "GUILD_TEXT") {
              const embed = new discord.MessageEmbed()
                .setDescription(`:pencil: ***Canal Creado***`)
                .addField("Canal", `${message}`, true)
                .addField("Nombre Del Canal", `${message.name}`, true)
                .addField("Tipo De Canal", "Canal De Texto", true)
                .setFooter({ text: `ID Del Canal: ${message.id}` })
                .setTimestamp()
                .setColor(color);

              if (message.parent && message.type !== "GUILD_CATEGORY")
                embed.addField(`Nombre Del Padre`, message.parent.name);
              ...
              const embed = new discord.MessageEmbed()
                ...
                .addField("Nombre Del Canal", `${message.name}`, true)
                .addField("Tipo De Canal", `${message.type}`, true)
                .setFooter({ text: `ID Del Canal: ${message.id}` })
                .setTimestamp()
                .setColor(color);
