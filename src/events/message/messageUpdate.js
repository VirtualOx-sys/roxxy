              ...
              if (oldMessage.content.length && newMessage.content.length) {
                if (oldMessage.content != newMessage.content) {
                  ...
                  const embed = new MessageEmbed()
                    .setAuthor(
                      `${newMessage.member.user.tag} | Mensaje Editado`,
                      newMessage.member.user.displayAvatarURL({ dynamic: true })
                    )
                    .setTimestamp()
                    .setDescription(
                      `
          ${newMessage.member} editó un mensaje en ${newMessage.channel}\n\n[¡Saltar al mensaje!](${newMessage.url})
        `
                    )
                    .addField("Antes", oldMessage.content)
                    .addField("Después", newMessage.content)
                    .setFooter({ text: `ID Del Miembro: ${newMessage.member.id}` })
