          ...
          if (logging.member_events.role_update == "true") {
            ...
            if (oldMember.roles.cache.size < newMember.roles.cache.size) {
              const roleAddembed = new discord.MessageEmbed()
                .setAuthor(
                  `${newMember.user.tag} | Rol Agregado`,
                ...
                .setDescription(
                  `**Rol Agregado**\n Rol: ${role}\n Usuario: ${newMember}\n\n ${newMember} se le dio el **${role.name}** Role.`
                );
            ...
            if (oldMember.roles.cache.size > newMember.roles.cache.size) {
              const roleRemoveembed = new discord.MessageEmbed()
                .setAuthor(
                  `${newMember.user.tag} | Rol Eliminado`,
                  newMember.user.displayAvatarURL({ dynamic: true })
                )
                .setTimestamp()
                .setColor(colorRed)
                .setFooter({ text: `ID: ${newMember.id}` })
                .setDescription(
                  `**Rol Eliminado**\n Rol: ${role}\n Usuario: ${newMember}\n\n El rol **${role.name}** se eliminó de ${newMember}`
                );
          ...
          if (logging.member_events.name_change == "true") {
            if (oldMember.nickname != newMember.nickname) {
              ...
              const nicknameEmbed = new discord.MessageEmbed()
                .setAuthor(
                  `${newMember.user.tag} | Actualización De Apodo`,
                  newMember.user.displayAvatarURL({ dynamic: true })
                )
                .setTimestamp()
                .setFooter({ text: `ID: ${newMember.id}` })
                ...
                .setDescription(
                  `**Actualización De Apodo**\n Se cambió el **apodo** de ${newMember}.`
                )
                .addField("Apodo", `${oldNickname} --> ${newNickname}`);
