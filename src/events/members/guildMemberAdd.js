...
module.exports = class extends Event {
  async run(member) {
    ...
    if (muteDoc) {
      const muteRole = member.guild.roles.cache.find((r) => r.name == "Silenciado");

      if (muteRole)
        member.roles
          .add(muteRole.id, ["Comando Mute / El usuario se fue y se reincorporó."])
          .catch(() => {});
        ...
        if (!arr.includes(member.id)) {
          ...
          if (day >= created) {
            ...
            if (
              ...
              await member
                .ban({ reason: "Cuenta Alternativa | Módulo Detector Alternativo" })
                .catch(() => {});
              action = "Banned";
            }
            if (
              ...
              await member
                .kick({ reason: "Cuenta Alternativa | Módulo Detector Alternativo" })
                .catch(() => {});
              action = "Kicked";
            }
            ...
            if (altLog) {
              if ((action && action === "Kicked") || action === "Banned") {
                const embedAlt = new discord.MessageEmbed()
                  .setColor("GREEN")
                  .setTitle(`Detector De ALT | ${member.user.tag}`)
                  .setDescription(
                    `**${action} ${
                      member.user.tag
                    } Del servidor.**\n\n**Razón:** Detector ALT\n\n__**Cuenta creada en:**__ ${moment(
