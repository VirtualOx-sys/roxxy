...
Strategy.prototype.userProfile = function (accessToken, done) {
  ...
  this._oauth2.get(
    ...
    function (err, body) {
      if (err) {
        return done(
          new InternalOAuthError("No se pudo obtener el perfil de usuario.", err)
        );
      }
      ...
      try {
        ...
        return done(new Error("No se pudo analizar el perfil de usuario."));
      }
  ...
  if (this._scope && this._scope.indexOf(scope) !== -1) {
    this._oauth2.get(
      ...
      function (err, body) {
        if (err)
          return cb(
            new InternalOAuthError("No se pudo obtener el usuario " + scope, err)
          );
        try {
          var json = JSON.parse(body);
        } catch (e) {
          return cb(new Error("No se pudo analizar el usuario " + scope));
        }
        cb(null, json);
      }
    );
