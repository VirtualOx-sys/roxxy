...
$(document).ready(() => {
  ...
  function generateInputFields(fields) {
    ...
    for (let i = 0; i < fields; i++) {
      $('.input-fields').append(`<div class="form-group row">
        ...
        <div class="col-sm-2">
          <button id="field-${i}-delete" class="btn btn-danger">Borrar</button>
        </div>
      </div>`);
      $(`#field-${i}-name`).keyup(() => {
    ...
    $('.input-fields').append(`<button id="add-field" class="btn btn-success">Agregar Campo</button>`);
    $('#add-field').click(e => {
      e.preventDefault();
      addField();
    });
  ...
  $('#title').keyup(() => {
    ...
    // preform checks
    if (title.length === 0) {
      addWarning(item, 'title', 'El título no puede estar vacío');
    } else {
  ...
  $('#icon').keyup(() => {
    ...
    let icon = item.val();
      ...
      if (number == "69") {
      addWarning(item, 'icon', 'URL invalida');
    } else {
  ...
  $('#author_icon').keyup(() => {
    ...
    if (number == "69") {
      addWarning(item, 'author_icon', 'URL no válida');
    } else {
