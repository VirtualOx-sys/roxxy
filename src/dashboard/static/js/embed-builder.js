...
window.onload = () => {
  ...
  let editorHolder = document.querySelector(".editorHolder"),
    ...
    json = {
      content:
        "Este es un mensaje Embed <a:Roxxy_Hecho:1015678763469648013> \nTodo es _Personalizable_ incluidos **Emojis**\nTambién puedes ~~no~~ hacer `esto`.```py\nY ('esto')```",
      embed: {
        title: "¡Hola! :wave:",
        description:
          "Puedes usar [enlaces](https://roxxy.es) o emojis :smile: :eyes:\n```\nY también bloques de código\n```",
        color: 4321431,
        ...
        author: {
          ...
          url: "https://roxxy.es",
          icon_url: "https://i.imgur.com/9L8rtuZ.png",
        },
        thumbnail: {
        ...
        footer: {
          text: "Este es un texto de pie de página",
          icon_url: "https://i.imgur.com/eh3TALt.png",
        },
        fields: [
          {
            name: "Field 1, ~~dolor~~",
          ...
          {
            name: "Field 2",
            value: "Puedes usar emojis personalizados  <:Roxxy:1015695739382407279>",
            inline: false,
          },
          {
            name: "Field Inline",
            value: "Los campos pueden estar en línea",
            inline: true,
          },
          {
            name: "Field Inline",
          ...
          {
            name: "Otro Field",
            value: "> No, no me olvidé de los bloques de código.",
            inline: false,
          },
        ],
      },
  ...
  let notif = document.querySelector(".notification"),
    ...
    allGood = (e) => {
      ...
      if (re)
        return error(
          `La URL debe comenzar con <code>https://</code> o <code>http://</code> en esta línea <span class="inline full">${makeShort(
            re[0],
            30,
            600
          )}</span>`
        );
      if (e.timestamp && new Date(e.timestamp).toString() === "Fecha Invalida")
        return error("La marca de tiempo no es válida");
      return true;
    },
    markup = (txt, opts) => {
      txt = txt
  ...
  buildGui = (object, opts) => {
    gui.innerHTML = `
                <div class="item content"><p class="ttle">Contenido Del Mensaje</p></div>
                <div class="edit">
                    <textarea class="editContent" placeholder="Contenido Del Mensaje" autocomplete="off">${encodeHTML(
                      object.content || ""
                    )}</textarea>
                </div>
                        ...
                        <div class="linkName">
                            <div class="editIcon">
                                ...
                                <input class="editAuthorLink" type="text" value="${encodeHTML(
                                  object.embed?.author?.icon_url || ""
                                )}" placeholder="URL Del Icono" autocomplete="off"/>
                            </div>
                            <div class="editName">
                    ...
                    <div class="item title inlineField">
                        <p class="ttle">Título</p>
                        <input class="editTitle" type="text" placeholder="Título" autocomplete="off" value="${encodeHTML(
                          object.embed?.title || ""
                        )}">
                    </div>
                    <div class="item description"><p class="ttle">Descripción</p></div>
                    <div class="edit">
                        <textarea class="editDescription" placeholder="Descripción Del Embed" autocomplete="off">${encodeHTML(
                          object.embed?.description || ""
                        )}</textarea>
                    </div>
                    <div class="item fields"><p class="ttle">Campos</p></div>
                    <div class="edit"></div>
                    ...
                    <div class="edit">
                        <div class="linkName">
                            ...
                            <div class="editName">
                                ...
                                )}" placeholder="Texto De Pie De Página" autocomplete="off" />
                            </div>
                        </div>
                        <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="field">
                        ...
                        <div class="fieldInner">
                            <div class="designerFieldName">
                                <input type="text" placeholder="Nombre Del Campo" autocomplete="off" value="${encodeHTML(
                                  f.name
                                )}">
                            </div>
                            <div class="designerFieldValue">
                                <textarea placeholder="Valor Del Campo" autocomplete="off">${encodeHTML(
                                  f.value
                                )}</textarea>
                            </div>
                        </div>
    ...
    document.querySelector(".addField").addEventListener("click", () => {
      ...
      if (arr.length >= 25)
        return error(`No puede tener más de 25 campos incrustados.`, 5000);
      arr.push({ name: "Nombre Del Campo", value: "Valor Del Campo", inline: false });
      ...
      e.addEventListener("input", (el) => {
        ...
        if (json.content) {
          if (json.content.length > 2000) {
            ...
            return error(`El contenido alcanzó los 2000 caracteres.`, 4000);
          }
        }
        ...
        if (json.embed.title) {
          if (json.embed.title.length > 256) {
            ...
            return error(`El título alcanzó los 256 caracteres`, 4000);
          }
        }
        ...
        if (json.embed.description) {
          if (json.embed.description.length > 2048) {
            ...
            return error(`La descripción alcanzó los 2048 caracteres`, 4000);
          }
        }
        ...
        fetch("https://tempfile.site/api/files", {
          ...
          .then((res) => {
            ...
            console.info(
              `Imagen (${
                res.link
              }) se eliminará en 5 minutos. Para eliminarlo ahora tengo que ${res.link.replace(
                "/files",
                "/del"
              )} e ingresa este código: ${res.authkey}`
            );
          })
          .catch((err) => `La solicitud a tempfile.site falló con el error: ${err}`);
      })
    );
  };
  ...
  document.querySelectorAll(".done .selectOptions .channel").forEach((e) =>
    e.addEventListener("click", (el) => {
      if (document.querySelector(".messageContent.empty+.container.empty"))
        return error(`No hay nada que enviar.`, 2500);
  ...
  $(document).on("click", "#save-custom", function (e) {
    ...
    if (!command) return error("¡Proporcionar un comando personalizado!", 4000);
    if (command.length > 30)
      return error("La longitud del comando supera los 30 caracteres.", 4000);

    error(
      'Comando guardado correctamente, si no se guardó, <a href="https://roxxy.es/embeds">leer aquí</a>',
      10000
    );
    e.preventDefault();
    $.ajax({
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", "#sendWebhook", function (e) {
      ...
      $.ajax({
        ...
        .done(function (response) {
          error("Webhook enviado correctamente", 4000);
        })
        .fail(function (xhr, status, message) {
          error("403 - Hubo un error al enviar su embed.", 6000);
        });
    });
  });
};
