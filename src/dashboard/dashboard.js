...
module.exports = async (client) => {
  ...
  passport.serializeUser((user, done) => {
    if (!user) return; // En serialización, si el valor recibido es falso, como vimos anteriormente, significa que la información no llegó en su totalidad, es decir, es una solicitud repetida y por lo tanto no obtuvo toda la información, por lo que debemos regresar para evitar errores de serialización
    ...
    new Strategy(
      ...
      async (accessToken, refreshToken, profile, cb) => {
        await process.nextTick(async () => {
          if (profile.guilds == undefined) return cb(null, false); // Cuando hay un error al obtener los servidores del usuario, la propiedad .guilds devuelve undefined, por lo que debemos señalar en la devolución de llamada que no ha llegado toda la información con cb (null, false)
  ...
  // Inicializamos el middleware del pasaporte.
    ...
    async function (req, res) {
      ...
      try {
        const login = new MessageEmbed()
          .setColor("GREEN")
          .setTitle(`Registros De Inicio De Sesión`)
          .setDescription(
            `\nUsuario: ${req.user.username}#${req.user.discriminator} \`(${
              req.user.id
            })\`\nHora: ${moment(new Date()).format(
              "dddd, MMMM Do YYYY HH:mm:ss"
            )} `
          );

        loginLogs.sendCustom({
          username: "Registros De Inicio De Sesión",
  ...
  app.get(
    ...
    passport.authenticate("discord", {
      failWithError: true,
      failureFlash: "¡Hubo un error al iniciar sesión!",
      failureRedirect: "/",
    }),
    async (req, res) => {
      const loginLogs = new Discord.WebhookClient({
  ...
  app.get("/commands", (req, res) => {
    res.send("Esta función no está disponible todavía.");
  });
  ...
  app.get("/logout", async function (req, res) {
    if (req.user) {
      ...
      const logout = new MessageEmbed()
        .setColor("RED")
        .setTitle(`Registros De Cierre De Sesión`)
        .setDescription(
          `\nUsuario: ${req.user.username}#${req.user.discriminator} \`(${
            req.user.id
          })\`\nHora: ${moment(new Date()).format(
      ...
      logoutLogs.send({
        username: "Registros De Cierre De Sesión",
        avatarURL: `${domain}/logo.png`,
        embeds: [logout],
      });
    }
  ...
  app.get("/url/:shortUrl", async (req, res) => {
    ...
    if (shortUrl == null) return res.send("URL proporcionada no válida");
  ...
  app.get("/paste/:pasteID", async (req, res) => {
    ...
    if (paste) {
      ...
      if (form && form.paste.length >= 1) {
        ...
        renderTemplate(res, req, "paste.ejs", {
          expires: moment(form.expiresAt).format("dddd, MMMM Del YYYY HH:mm:ss"),
          created: moment(form.createdAt).format("dddd, MMMM Del YYYY HH:mm:ss"),
          db: form,
          id: form._id,
          type: "form",
        });
  ...
  app.post("/apply/:guildID", checkAuth, async (req, res) => {
    ...
    if (channel) {
      ...
      for (let i = 0; db.questions.length > i; i++) {
        if (data[i + 1]) {
          form.paste.push(`Pregunta #${i + 1} - ${db.questions[i]}`);
          form.paste2.push(`${data[i + 1] || "Sin Respuesta"}`);

          embed = new MessageEmbed()
            .setTitle(`Se envió un nuevo formulario`)
            .setDescription(
              `**Link:** [${domain}/paste/${ticketID}](${domain}/paste/${ticketID})\n\n[o haga clic aquí](${domain}/paste/${ticketID})\n\n**ID Del Formulario**: \`${ticketID}\`\n\n**Presentado Por:** ${member} **(${
                member.user.tag
              } - ${member.id})**\n**Hora:** ${moment(new Date()).format(
                "dddd, MMMM Del YYYY HH:mm:ss"
              )}`
          ...
          form.paste.push(`Pregunta #${i + 1} - ${db.questions[i]}`);
          form.paste2.push(`Sin Respuesta`);
      ...
      member
        .sendCustom({
          embeds: [
            new MessageEmbed()
              .setColor("GREEN")
              .setFooter({ text: `Energizado Por https://roxxy.es` })
              .setTitle(`Solicitud #${ticketID}`)
              .setDescription(
                `¡Oye ${
                  member.user.username
                }! Su formulario fue enviado y listo para ser juzgado.\n\n**ID Del Formulario**: \`${ticketID}\`\n**Hora:** ${moment(
                  new Date()
                ).format("dddd, MMMM Del YYYY HH:mm:ss")}`
              ),
          ],
        })
        .catch(() => {});
      ...
      renderTemplate(res, req, "appealMain.ejs", {
        guild: guild,
        alert: `Su formulario ha sido recibido`,
        id: `Solicitud #${ticketID}`,
        app: db,
        settings: storedSettings,
      });
  ...
  app.post("/redeem/:guildID", checkAuth, async (req, res) => {
      ...
      .send({
        embeds: [
          new Discord.MessageEmbed()
            .setDescription(
              `**¡Felicidades!**\n\n**¡${guild.name}** Ahora es un gremio premium! ¡Gracias una tonelada!\n\nSi tiene alguna pregunta, póngase en contacto conmigo [aquí](${jsonconfig.discord})\n\n__**Recibo:**__\n**ID Del Recibo:** ${ID}\n**Fecha De Canje:** ${DDate}\n**Nombre Del Servidor:** ${guild.name}\n**ID Del Servidor:** ${guild.id}\n\n**Asegúrese de mantener esta información segura, es posible que la necesite si alguna vez desea reembolsar / transferir servidores.**\n\n**Caduca a las:** ${expires}`
            )
            .setColor("GREEN")
    ...
    const embedPremium = new Discord.MessageEmbed()
      .setDescription(
        `**Suscripción Premium**\n\n**${member.user.tag}** Canjeó un código en **${guild.name}**\n\n **ID Del Recibo:** ${ID}\n**Fecha De Canje:** ${DDate}\n**Nombre Del Servidor:** ${guild.name}\n**ID Del Servidor:** ${guild.id}\n**Etiqueta De Usuario:** ${member.user.tag}\n**ID Del Ususario:** ${member.user.id}\n\n**Caduca a las:** ${expires}`
      )
    ...
    renderTemplate(res, req, "redeemguild.ejs", {
      guild: guild,
      alert: `${guild.name} ¡Ahora es un gremio premium!`,
      settings: storedSettings,
    });
  });*/
  ...
  app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
    ...
    renderTemplate(res, req, "./new/mainpage.ejs", {
      guild: guild,
      alert: `El panel fue hecho por https://pogy.xyz`,
    ...
    if (data.prefix) {
      ...
      if (prefix.length > 5) {
        renderTemplate(res, req, "./new/mainpage.ejs", {
          ...
          leave1: leave1.length || 0,
          leave2: leave2.length || 0,
          alert: "La longitud del prefijo supera los 5 caracteres ❌",
          settings: storedSettings,
        });
        return;
      }
    ...
    if (
      ...
      if (checkIfEmbed) {
        ...
        if (!database) {
          ...
          renderTemplate(res, req, "./new/mainwelcome.ejs", {
            guild: guild,
            alert: `Asegúrate de guardar primero el canal de bienvenida. ❌`,
            settings: storedSettings,
            welcome: welcomeSettings,
            leave: leaveSettings,
          });
        ...
        if (data.leaveMessage) {
          if (data.leaveMessage.length > 2000) {
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Asegúrese de que la longitud del texto sea inferior a 2000 ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
      ...
      if (checkIfEmbed) {
        ...
        if (data.leave_author_name) {
          if (data.leave_author_name.length > 256) {
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Por favor, asegúrese de que la longitud del autor sea inferior a 200 ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
        ...
        if (data.leave_author_url) {
          if (
            ...
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Por favor, asegúrese de que esta sea una URL válida. ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
        ...
        if (data.leave_embedTitle) {
          if (data.leave_embedTitle.length > 200) {
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Asegúrese de que su título tenga menos de 200 caracteres ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
          ...
          renderTemplate(res, req, "./new/mainwelcome.ejs", {
            guild: guild,
            alert: `Por favor, asegúrese de incluir un título. ❌`,
            settings: storedSettings,
            welcome: welcomeSettings,
            leave: leaveSettings,
          });
        ...
        if (data.leave_embedTitleURL) {
          if (rgx.test(data.leave_embedTitleURL)) {
            ...
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Enlace proporcionado no válido ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
        ...
        if (data.leave_embedDescription) {
          if (data.leave_embedDescription.length > 2000) {
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Asegúrate de que la descripción tenga menos de 2000 caracteres. ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
          ...
          renderTemplate(res, req, "./new/mainwelcome.ejs", {
            guild: guild,
            alert: `Proporcione una descripción ❌`,
            settings: storedSettings,
            welcome: welcomeSettings,
            leave: leaveSettings,
          });
        ...
        if (data.leave_embedThumbnail) {
          if (rgx.test(data.leave_embedThumbnail)) {
            ...
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Proporcione una miniatura válida ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
        ...
        if (data.leave_embedFooter) {
          if (data.leave_embedFooter.length > 1048) {
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Asegúrese de que el pie de página tenga menos de 1000 caracteres ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
        ...
        if (data.leave_embedFooterIcon) {
          if (rgx.test(data.leave_embedFooterIcon)) {
            ...
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Icono de pie de página no válido ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
      ...
      if (checkIfEmbed) {
        ...
        if (data.embedImage) {
          if (
            ...
            renderTemplate(res, req, "./new/mainwelcome.ejs", {
              guild: guild,
              alert: `Proporcione una imagen válida ❌`,
              settings: storedSettings,
              welcome: welcomeSettings,
              leave: leaveSettings,
            });
  ...
  app.post("/dashboard/:guildID/moderation", checkAuth, async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(data, "auto_punish_disable")) {
      ...
      renderTemplate(res, req, "./new/mainmoderation.ejs", {
        guild: guild,
        alert: `Castigo automático ha sido deshabilitado ✅`,
        settings: storedSettings,
        mod: mod,
      });
    ...
    if (Object.prototype.hasOwnProperty.call(data, "dm_disable")) {
      ...
      renderTemplate(res, req, "./new/mainmoderation.ejs", {
        guild: guild,
        alert: `El mensaje de prohibición ha sido deshabilitado ✅`,
        settings: storedSettings,
        mod: mod,
      });
    ...
    if (
      ...
      if (message && message.length) {
        if (message.length > 1999) {
          renderTemplate(res, req, "./new/mainmoderation.ejs", {
            guild: guild,
            alert: `Proporcione un mensaje de menos de 2000 caracteres`,
            settings: storedSettings,
            mod: mod,
          });
          return;
  ...
  app.post("/dashboard/:guildID/autorole", checkAuth, async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(data, "autoroleDisable")) {
      ...
      renderTemplate(res, req, "./new/mainautorole.ejs", {
        ...
        settings: storedSettings,
        sticky: stickySettings,
        alert: "Desactivó con éxito el módulo autorole ✅",
      });
      return;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(data, "stickyroleDisable")) {
      ...
      renderTemplate(res, req, "./new/mainautorole.ejs", {
        ...
        settings: storedSettings,
        sticky: stickySettings,
        alert: "Desactivó con éxito el módulo Sticky Role ✅",
      });
      return;
    }
  ...
  app.post("/dashboard/:guildID/reactionroles", checkAuth, async (req, res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(data, "reset")) {
      ...
      if (resultsHeheLol === "0" || !results || !results.length) {
        renderTemplate(res, req, "./new/mainreactionroles.ejs", {
          guild: guild,
          alert: `El servidor actual no tiene ningún rol de reacción existente para eliminar.`,
          emojiArray: EmojiArray,
          settings: storedSettings,
        });
      ...
      renderTemplate(res, req, "./new/mainreactionroles.ejs", {
        guild: guild,
        alert: `Borrado con éxito ${resultsHeheLol} ${resultsHehe} ✅`,
        emojiArray: EmojiArray,
        settings: storedSettings,
      });
 ...
 */
    ...
    if (Object.prototype.hasOwnProperty.call(data, "send")) {
      ...
      if (channelValid) {
        ...
        renderTemplate(res, req, "./new/mainreactionroles.ejs", {
          guild: guild,
          alert: `Por favor, proporcione un canal válido`,
          emojiArray: EmojiArray,
          settings: storedSettings,
        });
      ...
      if (data.messageID) {
        try {
          ...
          renderTemplate(res, req, "./new/mainreactionroles.ejs", {
            guild: guild,
            alert: `Proporcióname un ID de mensaje válido`,
            emojiArray: EmojiArray,
            settings: storedSettings,
          });
          return;
      ...
      if (EmojiArray.includes(checkEmoji)) {
        ...
        renderTemplate(res, req, "./new/mainreactionroles.ejs", {
          guild: guild,
          alert: `Proporcióname un emoji válido`,
          emojiArray: EmojiArray,
          settings: storedSettings,
        });
      ...
      if (roleValid) {
        ...
        renderTemplate(res, req, "./new/mainreactionroles.ejs", {
          guild: guild,
          alert: `Por favor, proporcione un rol válido`,
          emojiArray: EmojiArray,
          settings: storedSettings,
        });
      ...
      renderTemplate(res, req, "./new/mainreactionroles.ejs", {
        guild: guild,
        alert: `Rol de reacción creado con éxito ✅`,
        emojiArray: EmojiArray,
        settings: storedSettings,
      });
  ...
  app.post("/report", async (req) => {
    if (req.body.type === "report") {
      ...
      const report = new MessageEmbed()
        .setColor("GREEN")
        .setTitle(`Informes Roxxy`)
        .setDescription(
          `¡Alguien acaba de reportar a un usuario!\n\nUsuario: ${req.body.name}\`(${req.body.id})\`\nUsuario Reportado: ${req.body.reported_user}\nID Del Usuario Reportado: ${req.body.reported_id}\nRazón: \`${req.body.reason}\`\nPruebas: ${req.body.proof}`
        );

      reportEmbed.sendCustom({
        username: "Informes Roxxy",
        avatarURL: `${domain}/logo.png`,
        embeds: [report],
      });
    }
  ...
  app.post("/contact", async (req) => {
    if (req.body.type === "contact") {
      ...
      const contact = new MessageEmbed()
        ...
        .setDescription(
          `¡Alguien acaba de contactarnos!\n\nUsuario: ${req.body.name}\`(${req.body.id})\`\nCorreo: ${req.body.email}\nMensaje: \`${req.body.msg}\``
        );

      contactEmbed.sendCustom({
        username: "Contacto Roxxy",
        avatarURL: `${domain}/logo.png`,
        embeds: [contact],
      });
    }
    ...
    if (
      ...
      if (data.ticketWelcomeMessage) {
        if (data.ticketWelcomeMessage.length > 1024) {
          ticketSettings.ticketWelcomeMessage = `¡Oye {user} Bienvenido a tu ticket!

Gracias por crear un ticket, el equipo de soporte estará contigo en breve. Mientras tanto, explique su problema a continuación`;
          renderTemplate(res, req, "./new/maintickets.ejs", {
            guild: guild,
            settings: storedSettings,
            ticket: ticketSettings,
            alert:
              "¡Asegúrate de que tu descripción tenga menos de 1024 caracteres!",
          });
...
Gracias por crear un ticket, el equipo de soporte estará contigo en breve. Mientras tanto, explique su problema a continuación`;
        ...
        renderTemplate(res, req, "./new/maintickets.ejs", {
          ...
          settings: storedSettings,
          ticket: ticketSettings,
          alert: "¡Asegúrate de incluir una descripción del ticket!",
        });
    ...
    if (Object.prototype.hasOwnProperty.call(data, "ticketDisable")) {
      ...
      renderTemplate(res, req, "./new/maintickets.ejs", {
        ...
        settings: storedSettings,
        ticket: ticketSettings,
        alert: "Desactivó con éxito el módulo de tickets ✅",
      });
      return;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
      ...
      if (ticketSettings.ticketToggle == "true") {
        if (
          ...
          if (reactionTitle.length > 200) {
            renderTemplate(res, req, "./new/maintickets.ejs", {
              ...
              settings: storedSettings,
              ticket: ticketSettings,
              alert: "¡Asegúrate de que tu título no sea tan largo! ❌",
            });
          ...
          if (reactionDescription.length > 1024) {
            renderTemplate(res, req, "./new/maintickets.ejs", {
              ...
              settings: storedSettings,
              ticket: ticketSettings,
              alert: "¡Asegúrate de que tu descripción no sea tan larga! ❌",
            });
          ...
          if (
            ...
            renderTemplate(res, req, "./new/maintickets.ejs", {
              ...
              settings: storedSettings,
              ticket: ticketSettings,
              alert: "Asegúrate de que tu embed sea correcto ❌",
            });
            return;
          }
          ...
          if (storedSettings.isPremium == "false") {
            ticketSettings.ticketFooter = "Motorizado Por Roxxy.es";
          } else {
            ...
            if (checkFooter2) {
              if (data.reactionfooterEmbed.length > 140) {
                renderTemplate(res, req, "./new/maintickets.ejs", {
                  ...
                  settings: storedSettings,
                  ticket: ticketSettings,
                  alert: "¡Asegúrate de que tu pie de página no sea tan largo! ❌",
                });
                return;
              }
          ...
          let reactionFooter = "Motorizado Por Roxxy.es";

          let footer = "Motorizado Por Roxxy.es";
          // eslint-disable-next-line no-unused-vars
          ...
          if (storedSettings.isPremium == "false") {
            ticketEmbed.setFooter({ text: `Motorizado Por Roxxy.es` });
          } else {
            if (checkFooter) {
          ...
          if (!data.messageID || data.messageID.length < 1) {
            renderTemplate(res, req, "./new/maintickets.ejs", {
              ...
              settings: storedSettings,
              ticket: ticketSettings,
              alert: "No se pudo encontrar el siguiente mensaje ",
            });
            return;
          }
          ...
          renderTemplate(res, req, "./new/maintickets.ejs", {
            ...
            ticket: ticketSettings,
            guild: guild,
            alert: "¡Reaccioné con éxito al mensaje! ✅",
          });
          return;
        }
        ...
        renderTemplate(res, req, "./new/maintickets.ejs", {
          ...
          ticket: ticketSettings,
          guild: guild,
          alert: "Envié con éxito el Embed ✅",
        });
        return;
      }
    }
    ...
    if (Object.prototype.hasOwnProperty.call(data, "useMessage")) {
      ...
      renderTemplate(res, req, "./new/maintickets.ejs", {
        ...
        ticket: ticketSettings,
        guild: guild,
        alert: "Cambiado con éxito a Ticketing de mensajes ✅",
      });
    ...
    if (Object.prototype.hasOwnProperty.call(data, "useReaction")) {
      ...
      renderTemplate(res, req, "./new/maintickets.ejs", {
        ...
        ticket: ticketSettings,
        guild: guild,
        alert: "Cambiado exitosamente a Ticketing Reacción ✅",
      });
      return;
    }
    ...
    renderTemplate(res, req, "./new/maintickets.ejs", {
      guild: guild,
      alert: `¡e han guardado los cambios! ✅`,
      ticket: ticketSettings,
      settings: storedSettings,
    });
  });
      ...
      if (storedSettings.isPremium == "true") {
        ...
        if (data.description) {
          if (data.description.length > 1024) {
            renderTemplate(res, req, "./new/mainsuggestions.ejs", {
              guild: guild,
              alert: `Asegúrate de que la descripción tenga menos de 1024 caracteres. ❌`,
              settings: storedSettings,
            });
            return;
          }
        ...
        if (data.footer) {
          if (data.footer.length > 1024) {
            renderTemplate(res, req, "./new/mainsuggestions.ejs", {
              guild: guild,
              alert: `Asegúrese de que el pie de página tenga menos de 1024 caracteres ❌`,
              settings: storedSettings,
            });
            return;
          }
  ...
  app.get("/dblwebhook", async (req, res) => {
    res.send(`¡La API de Top.gg está funcionando actualmente!`);
  });
  ...
  app.post("/dblwebhook", webhook.middleware(), async (req) => {
    ...
    const msg = new Discord.MessageEmbed()
      .setAuthor("Sistema De Votación", `${domain}/logo.png`)
      .setColor("#7289DA")
      .setTitle(`${apiUser.username} Ha Votado`)
      .setDescription(
        `¡Gracias **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) por votar a **Roxxy**!`
      );
    Hook.sendCustom(msg);
      ...
      voteUser.send({
        embeds: [
          new Discord.MessageEmbed()
            ...
            .setDescription(
              `¡Gracias **${apiUser.username}#${apiUser.discriminator}** (${
                apiUser.id
              }) por votar a **Roxxy**! \n\nVoto #${voteNumber + 1}`
            ),
        ],
  ...
  function collectMemoryStats() {
    ...
    metrics.gauge("USO DE CPU", cpu.usage());
    metrics.gauge(
      "Uso de RAM",
